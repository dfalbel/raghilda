---
title: "Using chunk attributes refine retrieval"
---

RAG requires retrieving relevant context.
In practice, a store is usually broad and mixed, and most chunks are irrelevant
...
You define a schema for chunk metadata (for example `artifact_id`, `note_type`,
...
for doc in [
    ...
    simple_store.insert(chunker.chunk_document(doc))
...
metadata, and this keeps ingestion flexible across labels, catalog entries, and
internal notes.
...
Finally, we step up to nested metadata and show how to filter it with dot-path
